...
namespace eRestaurantSystem.DAL
{
    ...
    internal class eRestaurantContext : DbContext
    {
        ...
        //when overriding the OnModelCreating(), it is important to remember to call the 
        //base method's implementaion before you exit the method

        //the ManyToManyNavigationPropertyConfiguration.Map method
        //lets you configure the tables and columns used for this
        //many to many relationship

        //it takes a ManyToManyNavigationPropertyConfiguration instance in which you specify
        //the column names by calling the MapLeftKey, MapRightKey, and ToTable methods

        //the "left" key is the one specified in the HasMany method the "right" key is the one 
        //specified in the WithMany method

        //this navigation replaces the sql associated table that breaks up a many to many relationship
        //this technique should only be used if the assocaited table in sql has only a compound PK and
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            base.OnModelCreating(modelBuilder); //don not remove
        }
    }
}
